...
Meteor.startup(function () {
  ...
  Router.route('/tel', 
    function(){
      var route = this.request.url.replace(/\/tel/, "");
      if (route == "") route = "/";
      console.log( "route", this.request.url, "redirect", route);
      // this.redirect(route);
      Router.go(route);
    });
    */
  ...
  Router.map(function() {
    // -------------------------------------------- MedBook Special routes  -------------------------------------------- //
   
    // Lop off the /tel and redo the route //
    this.route('tel',  { path: '/tel', action: function(){ Router.go("/"); } });
    this.route('tel2', { path: '/tel/:remainder(*)',
        action: function(){ Router.go("/" + this.params.remainder); }
      }
    );
